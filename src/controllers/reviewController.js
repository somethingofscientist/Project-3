...
const createReview = async function (req, res) {
  try {
    ...
    const newObj = {}
    newObj.bookId = newbookId
    newObj.reviewedAt = bookData.reviewedAt
...
const updateReview = async function (req, res) {
  try {
    ...
    // _____________bookId validation_________________//
...
//_______________reviewId validations_________________//
  ...
  if(review){
    ...
    //______________reviewedBy validations_________________//
       ...
       let updatedreview = await reviewModel.findOneAndUpdate({_id:reviewId},{data},{new:true})
...
const deleteReview = async function (req, res) {
  try {
    ...
    { return res.status(400).send({ status: false, msg: "this review is not for this book" }) }

    await reviewModel.findOneAndUpdate({ _id: reviewId }, { $set: { isDeleted: true } }, {new:true})
    validBook.review = validBook.review - 1
